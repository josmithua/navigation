...
import { NavigationContext, NavigationContextType } from 'navigation-react';
import BackButton from './BackButton';
type SceneProps = {
  crumb: number;
  sceneKey: string;
  renderScene: (state: State, data: any) => React.ReactNode;
  crumbStyle: any;
  unmountStyle: any;
  hidesTabBar: any;
  title: (state: State, data: any) => string;
  popped: (key: string) => void;
  navigationEvent: NavigationContextType;
};
type SceneState = { navigationEvent: NavigationContextType };
...
class Scene extends React.Component<SceneProps, SceneState> {
  ...
  onBeforeNavigate(_state, _data, url: string) {
    ...
    if (url.split('crumb=').length - 1 !== crumb || Platform.OS === 'android') return true;
    ...
    if (!changed && crumb < crumbs.length) {
      ...
      changed =
        state !== latestState || Object.keys(data).length !== Object.keys(latestData).length;
      for (var key in data) {
  ...
  render() {
    ...
    var { state, data } = navigationEvent
...
var NVScene = requireNativeComponent<any>('NVScene');
