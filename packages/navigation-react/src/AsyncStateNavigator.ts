...
class AsyncStateNavigator extends StateNavigator {
  ...
  navigateLink(
    url: string,
    historyAction: 'add' | 'replace' | 'none' = 'add',
    history = false,
    ...
    this.stateNavigator.navigateLink(
      url,
      historyAction,
      history,
      (stateContext, resumeNavigation) => {
        suspendNavigation(stateContext, () => {
          var asyncNavigator = new AsyncStateNavigator(
  ...
  private suspendNavigation(
    asyncNavigator: AsyncStateNavigator,
